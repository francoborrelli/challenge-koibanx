...
import { DIUsersContainer } from '../../src/infraestructure/DIUsersContainer';
...
import { DITokenContainer } from '../../src/infraestructure/DITokenContainer';
...
const insertUsers = async (users: NewCreatedUser[]) => {
  for (const user of users) {
    await DIUsersContainer.getCreateUserUseCase().execute({ ...user, password: hashedPassword });
  }
};

const insertTask = async (tasks: NewIUploadTaskDocData[]) => {
  for (const task of tasks) {
    await DITaskContainer.getCreateTaskUseCase().execute(task);
  }
};

const adminAccessToken = DITokenContainer.getLoginUseCase().execute(admin.email, admin.password);
